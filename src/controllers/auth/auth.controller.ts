...
export const signup = async (req: Request, res: Response) => {
  try {
    ...
    if (!["NGN", "USD"].includes(preferredCurrency)) {
      return res
        .status(400)
        .json(errorResponse("Preferred currency must be NGN or USD."));
    }
    ...
    if (existingUser) {
      return res
        .status(409)
        .json(errorResponse("Email or username is already in use."));
    }
    ...
    res
      .status(500)
      .json(errorResponse("Server error. Please try again later."));
  }
};
...
export const loginWithPasscode = async (req: Request, res: Response) => {
  try {
    ...
    if (!email || !passcode) {
      return res
        .status(400)
        .json(errorResponse("Email and passcode are required."));
    }
    ...
    if (!user || !user.passcode) {
      return res
        .status(404)
        .json(errorResponse("User not found or passcode not set."));
    }
...
export const updateUserProfile = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!userId) {
      return res
        .status(401)
        .json(errorResponse("Unauthorized: No user ID found."));
    }
    ...
    if (updateData.email || updateData.password) {
      return res
        .status(400)
        .json(errorResponse("Email and password cannot be updated here."));
    }
